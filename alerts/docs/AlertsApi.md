...
with alerts.ApiClient(configuration) as api_client:
    ...
    api_instance = alerts.AlertsApi(api_client)
...
with alerts.ApiClient(configuration) as api_client:
    ...
    state = alerts.State() # State | Optional parameter to match a specific alert state. If not specified, it defaults to `trigger`. (optional)
