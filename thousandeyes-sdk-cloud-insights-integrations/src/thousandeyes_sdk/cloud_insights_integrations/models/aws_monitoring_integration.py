...
class AwsMonitoringIntegration(BaseModel):
    """
    AwsMonitoringIntegration
    """ # noqa: E501
    id: StrictStr = Field(description="The unique ID for the AWS inventory or flow logs monitoring integration.")
    name: StrictStr = Field(description="The name of the AWS inventory or flow logs monitoring integration.")
    role_arn: StrictStr = Field(description="The ARN of the AWS role to be monitored.", alias="roleArn")
    external_id: StrictStr = Field(description="The external ID associated with the account group.", alias="externalId")
    monitoring_type: StrictStr = Field(description="The type of monitoring integration.", alias="monitoringType")
    sns_topics_arns: Optional[List[StrictStr]] = Field(default=None, description="The array of SNS topic ARNs. Relevant only for flow logs monitoring integrations.", alias="snsTopicsArns")
    links: Optional[SelfLinks] = Field(default=None, alias="_links")
    __properties: ClassVar[List[str]] = ["id", "name", "roleArn", "externalId", "monitoringType", "snsTopicsArns", "_links"]

    @field_validator('monitoring_type')
    def monitoring_type_validate_enum(cls, value):
        """Validates the enum"""
        if value not in set(['inventory-monitoring', 'flow-logs-monitoring', 'unknown']):
            raise ValueError("must be one of enum values ('inventory-monitoring', 'flow-logs-monitoring', 'unknown')")
        return value
    ...
    def from_dict(cls, obj: Optional[Dict[str, Any]]) -> Optional[Self]:
        ...
        _obj = cls.model_validate({
            ...
            "snsTopicsArns": obj.get("snsTopicsArns"),
