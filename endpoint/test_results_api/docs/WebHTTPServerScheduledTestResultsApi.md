# test_results_api.WebHTTPServerScheduledTestResultsApi
...
with test_results_api.ApiClient(configuration) as api_client:
    ...
    api_instance = test_results_api.WebHTTPServerScheduledTestResultsApi(api_client)
    ...
    expand = [test_results_api.Expand()] # List[Expand] | This parameter is optional and determines whether to expand resources related to test results. By default, no expansion occurs when this query parameter is omitted. To expand a specific resource, such as \"header,\" append `?expand=header` to the query. (optional)
