...
**nic_model** | **List[str]** | Returns only agents whose NIC model is an exact, case-sensitive match for one of the provided values. | [optional] 
**nic_driver_version** | **List[str]** | Returns only agents whose NIC driver version is an exact, case-sensitive match for one of the provided values. | [optional] 
