...
class AgentSearchFilters(BaseModel):
    ...
    nic_model: Optional[List[StrictStr]] = Field(default=None, description="Returns only agents whose NIC model is an exact, case-sensitive match for one of the provided values.", alias="nicModel")
    nic_driver_version: Optional[List[StrictStr]] = Field(default=None, description="Returns only agents whose NIC driver version is an exact, case-sensitive match for one of the provided values.", alias="nicDriverVersion")
    ...
    __properties: ClassVar[List[str]] = ["id", "agentName", "computerName", "username", "userPrincipalName", "platform", "osVersion", "nicModel", "nicDriverVersion", "serialNumber", "locationCountryISO", "locationSubdivision1Code", "locationCity", "licenseType", "anyConnectDeviceId"]
