...
class SslCert(BaseModel):
    ...
    is_fetch_date_in_valid_cert_date_range: Optional[StrictStr] = Field(default=None, description="True when certificate fetch date is within the valid certificate date range, false otherwise", alias="isFetchDateInValidCertDateRange")
    has_valid_signing_cert: Optional[StrictStr] = Field(default=None, description="This field is implicitly true; it is output only when false. false indicates this certificate was missing a valid signing certificate in the chain.", alias="hasValidSigningCert")
