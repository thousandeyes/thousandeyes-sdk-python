...
class EndpointHttpServerInstantTest(BaseModel):
    ...
    endpoint_agent_labels: Optional[List[StrictStr]] = Field(default=None, description="Deprecated. Use `tagIds` instead.  List of endpoint agent label IDs (obtained from `/endpoint/labels` endpoint), required when `agentSelectorType` is set to `agent-labels`. ", alias="endpointAgentLabels")
    tag_ids: Optional[List[StrictStr]] = Field(default=None, description="List of tag IDs (obtained from `/tags` endpoint).", alias="tagIds")
    ...
    __properties: ClassVar[List[str]] = ["agentSelectorType", "agents", "endpointAgentLabels", "tagIds", "maxMachines", "testName", "ipVersion", "authType", "hasPathTraceInSession", "httpTimeLimit", "protocol", "username", "sslVersionId", "tcpProbeMode", "verifyCertificate", "url", "hasPing", "hasTraceroute", "networkMeasurements", "targetResponseTime", "password"]
